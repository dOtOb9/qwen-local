...
- その後、実際の発生時刻からの経過時間を使ってP波/S波の到達半径を
  km/s単位で本当に計算するリアルタイム版に作り直した。

## 地震速報機能を完全に削除(2026-09-12)

観測点1点(震源)だけでは実際の揺れの伝わり方を正確に再現できず、
シミュレーションが凝る割に本質的な価値(自分の秒読み・現在地での揺れ予測)を
提供できないことに気づき、ユーザーの判断で「地震速報を入れるのは愚策だった」
として機能全体を削除。以下を削除・巻き戻し:
- `src/components/EarthquakeView.tsx`(フルスクリーン地図・波紋シミュレーション)
- `src/components/Dock.tsx`(タブがチャット1つだけになり意味を失ったため道連れで削除)
- `src/lib/earthquake.ts`(P2P地震情報 WebSocket/history クライアント)
- `src/lib/japanProjection.ts`、`src/assets/japan-map.svg`(独自較正した地図投影)
- `App.tsx`から地震関連のstate/effect/UI分岐を除去し、元のサイドバー+チャットの
  単一画面構成に戻した
- `@tauri-apps/plugin-notification` / `tauri-plugin-notification`
  (地震通知专用だったため、他に利用箇所がなく削除)、および
  `capabilities/default.json`の`notification:default`権限
- `npx tsc --noEmit`と`cargo check`でクリーンビルドを確認後、コミット・push済み
  (コミット`81c74c5`)

得られた教訓: 単一のAPI(震源座標のみ)から「リアルタイム物理シミュレーション」を
