...
## 事故: Issue #2のブランチが無関係な他機能を巻き込んでいた(2026-09-12)

4回目の実行(Issue #2, モデル切り替え)は`max-turns 60`でも上限到達で
失敗扱いになったが、実装・push・ラベル変更・compare URLコメントまでは
完了していた。ただし内容を確認したところ、**別セッションが実装した
地震アラート機能(P波/S波シミュレーション、時刻表示)を巻き込んで
リバートしてしまっていた**(`EarthquakeView.tsx`と
`TaskSheets/2026-09-10-backend-frontend-plan.md`が無関係に変更されていた)。
`git add -A`は使っていなかったはずだが、エージェント自身が「ついでに」
これらのファイルを編集してcommitしていたと見られる。

**対応**: 該当2ファイルを`origin/master`の内容に戻すcommitを
`agent/issue-2-model-switcher`ブランチに直接push(compare URLで
PRを作る前にリポジトリ側で修正)。マージ前に必ず気づける状態にした。

**再発防止**: `agent-dev-loop.yml`のプロンプトに以下を追加:
- 「このIssueの実装に直接必要なファイルだけを変更する。並行開発中の
  他機能のコードやTaskSheetsを、気になっても勝手にリファクタ・削除・
  簡略化しない」という明示的な指示
- commit前に`git diff --stat`で変更ファイル一覧を確認し、無関係な
  ファイルが混ざっていたら`git checkout --`で戻す手順を追加

**教訓**: 複数セッションが同じmasterに並行commitしている環境では、
「`git add -A`を使わない」だけでは不十分で、「エージェント自身が
スコープ外のファイルを能動的に編集しない」ことも明示的に指示する
必要がある。人間側のレビュー(compare URL→PR作成前)がまさにこの種の
事故を捕まえるための最後の砦になっている。

## cronが動き出し、CI Doctorが実際に機能した(2026-09-12〜13)

`agent-dev-loop.yml`を編集しないまま数時間放置したところ、cronが
15:32・20:33・(翌日)04:48と3回連続で発火した。「編集のたびに認識タイマーが
リセットされる」という仮説が正しかったと確認できた。

このうち20:33の実行はIssue #4(Google Calendar連携)を実装・build確認・push
まで完了したが、`--max-turns 60`の上限(61ターン目)で後片付け(compare URL
組み立て・ラベル変更・Issueコメント)の直前に強制終了した。これを検知した
**CI Doctorが設計通りに動作**: 原因を`gh run view --log-failed`で特定し、
`TaskSheets/2026-09-12-ci-failure-agent-dev-loop.md`に診断結果と具体的な
修正案(`max-turns`を100に上げる案、将来的な代替設計案も含む)を記録し、
Issue #4に人間向けのcompare URLコメントも代わりに投稿した。ワークフロー
ファイル自体は変更できないという制約下で意図通りの「人間への引き継ぎ」が
機能した実例になった。

対応: CI Doctorの提案通り`max-turns`を100に引き上げ、`agent-dev-loop.yml`
にも`show_full_output: true`を追加。Issue #4のブランチは最新masterとの
間で衝突が無いことを確認済み(テストマージで検証)。

追記: 直前の15:32〜15:34の別の失敗(Agent Dev Loop本体とCI Doctorの
両方が同時間帯に`is_error:true`で失敗)の原因も、CI Doctor側のログ
(`show_full_output`が既に有効だったため中身が見えた)から特定できた。
`"error": "rate_limit"`, `"api_error_status": 429` — **Anthropic APIの
レート制限**だった。この時間帯、対話セッション(私)がこのプロジェクトの
デバッグ作業で頻繁にAPIを叩いており、そこにcronのAgent Dev Loopと
CI Doctorが同時に動こうとして、同じClaudeサブスクリプションの割り当てが
一時的に埋まったのが原因と見られる。設計上の欠陥ではなく一過性の負荷
問題で、実際その後(20:33以降)は正常に動作している。リトライ処理は
特に追加せず(次のcronサイクルで自然に回復するため)、現状は許容する。

## 現状(2026-09-13時点)

- Issue #1〜#3: 実装・PR・マージ・クローズまで完了
- Issue #4: 実装・push完了、人間がcompare URLからPR作成するのを待っている状態
- ハーネス(`agent-dev-loop.yml`)・CI Doctor(`ci-doctor.yml`)・PRごとの
  Windows実ビルド確認(`windows-pr-check.yml`)がひととおり機能することを
  確認できた
- 残るバックログは無し。次に`agent-ready`にする種をどうするかは未定
  (ユーザー主導で新規Issueを起票するか、コード読解して改善案を出すか)
